...
4. 如果提示框没有“打开”，点击“完成”，然后打开 **系统设置 → 隐私与安全性**。在“安全性”区域
   ...
   也可以在 Finder 中按住 Control 点按应用，选择 **打开**，再在确认框中点 **打开**。
5. 应用启动后会在用户目录部署或升级主题引擎。若 Codex 正在运行，按提示先退出并选择重试。
...
菜单栏中的“检查更新”（如果当前版本提供）只在用户点击时访问 GitHub Releases，不会后台轮询，
也不会静默下载或替换应用。
...
仍没有按钮时，退出系统设置后重新打开，或使用 Finder 的 Control-点按 → 打开。不要关闭系统
安全功能，也不要从网上复制未经核对的终端命令。
